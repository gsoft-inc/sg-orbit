...
A Provider must be the root component of your application. All other Orbit UI components rely on the `ThemeProvider` to define the theme and color scheme they need to render accurately.

If you need to access the current theme you can do so, by using the `useThemeContext` hook.

> The default theme is *apricot*, *desktop* theme is only available to showcase how multi-theme works in Orbit.
...
<Preview filePath="/theme-provider/docs/ColorSchemePicker" />
