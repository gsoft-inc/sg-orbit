...
<ComponentInfo
    usage={"import { ThemeProvider, ThemeContext, useTheme } from \"@orbit-ui/react-components\""}
...
The closest `ThemeContext` can be accessed with the `useTheme` hook. Once you have an hold on the context, you can access is values and update the `colorScheme`.
