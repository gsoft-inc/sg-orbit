...
import { PureTextInput, TextInput } from "@react-components/text-input";
import { RefLogger, createComponentSection, paramsBuilder } from "@utils";
import Imports from "./imports.mdx";

<Meta
    title={createComponentSection("Text Input")}
...
<Props
    componentsDefinitions={[
        { displayName: "TextInput", component: PureTextInput }
    ]}
/>
...
<Preview>
    <Story name="value">
        <TextInput defaultValue="Orbit" />
    </Story>
</Preview>

### Focused

A text input can show a user is currently interacting with it.

<Preview>
    <Story name="focused">
        <TextInput focus placeholder="Search..." />
    </Story>
</Preview>
...
<Preview>
    <Story name="readonly">
        <TextInput readOnly value="Orbit" />
    </Story>
</Preview>
...
A text input can be formatted with an [icon](?path=/docs/materials-icons--page). Specify the icon to use with a [shorthand value](?path=/docs/getting-started-shorthand-props--page).
...
A few API functions are available for a text input component. To use an API, first get a [ref](https://reactjs.org/docs/refs-and-the-dom.html) on the text input component, then call the API function.

### focus

A text input can be focused programmatically.
...
A text input text can be [selected](https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement/select) programmatically.
