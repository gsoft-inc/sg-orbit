...
import { DEFAULT_SIZE, RemoteSearchInput, searchInputResult, useDefaultResultsFetcher } from "@react-components/search-input";
...
<InstallationReactComponents componentFolder="search-input" imports={<Imports />} />
...
export const dataFetcher = useDefaultResultsFetcher("https://wft-geo-db.p.rapidapi.com/v1/geo/countries?limit=5", "namePrefix", { requestOptions: API_REQUEST_OPTIONS });
...
```javascript dark
const dataFetcher = useDefaultResultsFetcher(
...
<FilePreview filePath="/search-input/stories/remote-search-input/components/controlled-remote-search-input">
