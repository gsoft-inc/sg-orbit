import { Apollo11, ET, TheMartian } from "./assets";
...
import { InnerModal, InnerModalTrigger, Modal, ModalTrigger } from "@react-components/dialog";
...
A modal can have an illustration.

<Preview scope={{ Apollo11 }}>
    <Story name="illustration">
        <ModalTrigger>
            <Button>Open</Button>
            <Modal>
                <Illustration>
                    <Image src={Apollo11} alt="Apollo 11 poster" />
                </Illustration>
                <Heading>Apollo 11</Heading>
                <Content>
                    <Paragraph>Apollo 11 is a 2019 American documentary film edited, produced and directed by Todd Douglas Miller. It focuses on the 1969 Apollo 11 mission, the first spaceflight from which men walked on the Moon.</Paragraph>
...
A modal can offer a choice between 2 propositions.
...
A modal can have a single button
...
A modal can doesn't have to be dismissable.
...
<Preview filePath="/dialog/docs/ControlledModal" />
...
<ComponentInfo
    ...
    slots={["illustration", "heading", "header", "content", "footer", "button", "button-group"]}
    compact
/>
