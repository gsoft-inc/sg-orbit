...
import { JsxElement, SlotProps, StyledComponentProps, mergeProps, omitProps, useStyleProps, useStyledSystem } from "../../shared";
...
export type HtmlElement<T extends JsxElement<T>> = SlotProps & StyledComponentProps<T> & {
    as?: ElementType;
};
...
export function htmlElement<T extends JsxElement<T>>(name: string, elementType: T, className?: string) {
    return forwardRef<ElementRef<T>, HtmlElement<T>>((props, ref) => {
        ...
        const {
            ...
            children,
            ...rest
        } = omitProps(useStyledSystem(mergedProps), ["slot"]);
