...
A popover can be rendered in *persistent mode* which will force the user to explicitly dismiss the popover with a targeted call to action.

The call to action can dismiss the popover by calling a `close` function which can be retrieved from either `usePopoverTriggerContext` or the render props.

<Preview>
    <Story name="persistent">
        <PopoverTrigger persistent>
            {({ close }) => (
                <>
                    <Button>Trigger</Button>
                    <Popover>
