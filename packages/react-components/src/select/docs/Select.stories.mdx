...
A select items can be provided with an array instead of using the granular components.

<Preview>
    <Story name="data items">
        <Select placeholder="Planets" aria-label="Planets">
...
A select can render his items based on the `selectedKey` value.
...
<Preview>
    <Story name="render props">
        <Select placeholder="Planets" aria-label="Planets">
            {({ selectedKey }) =>
                ["Earth", "Jupiter", "Mars", "Mercury", "Neptune", "Saturn", "Uranus"].map(x => (
                    <Item key={x.toLowerCase()} className={cx({ "primary-600": selectedKey === x.toLowerCase() })}>
                        {x}
                    </Item>
                ))
            }
