...
import { HiddenSelect, InnerItem, InnerSection, Item, Section } from "@react-components/placeholders";
...
A select open direction can vary.

<Preview>
    <Story name="direction">
        <Select direction="top" placeholder="Planets" aria-label="Planets">
            <Item key="earth">Earth</Item>
            <Item key="jupiter">Jupiter</Item>
            <Item key="mars">Mars</Item>
            <Item key="mercury">Mercury</Item>
...
Some use cases requires to render a select with a custom trigger. Orbit helps by providing a `useSelect` hook.
