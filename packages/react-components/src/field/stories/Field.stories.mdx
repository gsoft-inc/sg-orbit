...
import { getSizeClass } from "@react-components/shared";
...
A field context is also available through a render prop function.

<Preview>
    <Story name="render props">
        <Field>
            {
                ({ labelId, inputId, size, labelClassName }) => (
                    <Fragment>
                        <label id={labelId} htmlFor={inputId} className={`${labelClassName} ${labelClassName}-${getSizeClass(size)}`}>
                            Space ID
                        </label>
                        <TextInput />
                        <HelpMessage>Your space id is a 9-digit number available the top right corner of your membership card.</HelpMessage>
