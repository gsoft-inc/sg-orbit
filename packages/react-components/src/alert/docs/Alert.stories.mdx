import { Alert, AlertTemplate, ErrorAlert, InfoAlert, InnerAlert, PositiveAlert, WarningAlert } from "@react-components/alert";
...
An alert can accept an action [button](?path=/docs/button--default-story).
...
<ArgsTable of={AlertTemplate} sort="alpha" />
...
<ComponentInfo
    usage={"import { ErrorAlert } from \"@orbit-ui/react-components\";"}
    slots={["content", "button"]}
    compact
/>
