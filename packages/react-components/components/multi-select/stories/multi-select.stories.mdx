...
<Meta
    ...
    parameters={
        paramsBuilder()
            .docsContainer(<ReactComponentsContainer sourcePath="/packages/react-components/components/multi-select/src" />)
            .build()
    }
/>
...
<Installation componentName="multi-select" imports={<Imports />} />
