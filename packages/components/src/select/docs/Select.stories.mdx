...
- If you provide an `aria-label` attribute, make sure its value is in sync with the selected item, otherwise screen readers won't be able to anounce the new selected item.
...
<Preview>
    <Story name="trigger icon">
        <Select icon={<MagnifierIcon />} placeholder="Planets">
            <Item key="earth">Earth</Item>
            <Item key="jupiter">Jupiter</Item>
            <Item key="mars">Mars</Item>
            <Item key="mercury">Mercury</Item>
...
<ComponentInfo
    usage={"import { Select } from \"@sharegate/orbit-ui\";"}
...
<ComponentInfo
    usage={"import { HiddenSelect } from \"@sharegate/orbit-ui\";"}
