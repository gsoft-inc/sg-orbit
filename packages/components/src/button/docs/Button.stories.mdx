...
- Be positionned in consistent locations.

### Accessibility

- Use the disabled prop to set the disabled state of the button.

### Button vs Link vs Switch
...
A button can contain [icons](?path=/docs/icon-gallery--page). We should avoid using icons on small buttons.
...
Used to disable a button that isn’t usable.
