...
<ComponentInfo
    usage={"import { Grid, repeat, minmax, fitContent } from \"@sharegate/orbit-ui\";"}
...
<Preview scope={{ ApricotTheme }}>
    <Story name="repeat function">
        <Grid
            ...
            gap={2}
        >
            {ApricotTheme.colors.common.purple.map(x => <Div backgroundColor={x} key={x} />)}
            {ApricotTheme.colors.common.alert.map(x => <Div backgroundColor={x} key={x} />)}
            {ApricotTheme.colors.common.green.map(x => <Div backgroundColor={x} key={x} />)}
        </Grid>
    </Story>
</Preview>
...
<ComponentInfo
    usage={"import { Grid } from \"@sharegate/orbit-ui\";"}
