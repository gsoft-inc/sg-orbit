...
<Preview scope={{ ApricotTheme }}>
    <Story name="wrap">
        <Flex wrap="wrap" gap={1} width={18}>
            ...
            {ApricotTheme.colors.common.alert.map(x => <Div backgroundColor={x} width={6} height={6} key={x} />)}
            {ApricotTheme.colors.common.green.map(x => <Div backgroundColor={x} width={6} height={6} key={x} />)}
        </Flex>
    </Story>
</Preview>
