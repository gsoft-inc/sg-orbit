...
Multiple [npm packages](?path=/docs/packages--page) compose Orbit. Based on which type of application you're building you'll choose a different composition.

## React bundle

For a React application, the easiest way to use Orbit is by installing the [@orbit-ui/react](https://www.npmjs.com/package/@orbit-ui/react) bundle. The bundle will install the [core](?path=/docs/packages--page#core) and [React](?path=/docs/packages--page#components) packages of Orbit in 5 easy steps.
...
```javascript
import { AddIcon, Button, Select, ThemeProvider } from "@orbit-ui/components";
```
...
import { ThemeProvider } from "@orbit-ui/components";
...
Before installing any [React components packages](?path=/docs/packages--page#components), you must install the [core packages](?path=/docs/packages--page#core). You can pick and choose the *core packages* to install but we do recommend installing all of them.
...
Once the *core packages* are installed, you can optionally install [React components packages](?path=/docs/packages--page#components).

#### Install packages

```bash
npm install @orbit-ui/components
```

### Import styles

```css
@import "~@orbit-ui/components/dist/index.css";
```
