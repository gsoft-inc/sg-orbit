...
In Orbit, component placeholders are fulfilled through his children and slots are defined [implicitly](#implicit-slots) or [explicitly](#explicit-slots) on the child component.
...
For native HTML elements, instead of specifying an explicit slot directly on the native element, we recommend using [Orbit HTML elements](?path=/docs/html-anchor--example).
