...
When a CSS pseudo-classes is not supported by Orbit style props, we recommend that you use CSS classes instead.

The following user action pseudo-classes are supported by some style props and behaves like their pseudo CSS counterparts:
...
> Not all style props supports user action pseudo-classes. Find out which props support user action pseudo-classes in the [reference](#reference) section.
...
For text elements, prefer a [`<Text>`](?path=/docs/text--size) or [`<Paragraph>`](?path=/docs/paragraph--size) component to a `<Span>` or a `<Div>`.

## Layout containers

To faciliate layouts, Orbit provides specialized components for [Grid](https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Grids) and [Flex](https://developer.mozilla.org/en-US/docs/Glossary/Flex_Container) layout containers.
...
Any component can accept style props. To accept style props, the component interface must extends the `StyledComponentProps` interface and either forward all the style props to an Orbit components or call the `useStyledSystem` hook.
