...
Multiple [packages](?path=/docs/getting-started-packages--page) compose Orbit UI. Based on which type of application you're building you'll choose a different composition.
...
Before installing any [React components packages](?path=/docs/getting-started-packages--page#react-components), the [core packages](?path=/docs/getting-started-packages--page#core) must be installed.
...
If you choose to do so, 3 options are available:
...
When it's done, import the React components that you need and the Semantic UI theme:
...
Orbit icons depends on [SVGR](https://github.com/gregberge/svgr) to transform SVG icons into React components. To use SVGR, your project babel config must be updated to add [babel-plugin-named-asset-import](https://www.npmjs.com/package/babel-plugin-named-asset-import) plugin with the following configuration:
