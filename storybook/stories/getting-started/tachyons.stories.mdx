...
Orbit provide a [custom version](#customization) of Tachyons as a foundation of utility building blocks to handle a range of common use cases and help us avoid writing custom styles. When we need to add some margin or padding, rather than adding a new selector specific to that use case, we can use utilities. This helps us reduce the number of unique property-value pairs, and helps us keep more consistent styles across our products.
...
Orbit foundation [spacing scale](?path=/docs/materials-spacing--page) have been implemented as Tachyons CSS classes. Tachyons spacing scale have been increased from _7_ to _13_ values.

This means that all the paddings and margins variants are available from `0 to 13`. Please refer to the [spacing page](?path=/docs/materials-spacing--page) for an exhaustive list of all the available classes.
...
For any [products](?path=/docs/materials-colors--page#products) colors the same naming scheme is valid but with a [_product-name_] instead of a [_color-name_]:

- [_product-name_]-[_50-900_] (ex. `apricot-50`)

_Primary_ classes are also available:
